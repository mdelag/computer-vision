...
async function loadModels() {
    try {
        ...
        try {
            ...
            showWarning('WebGL not available. Using CPU backend which may be slower.');
        }
        ...
        try {
            ...
            console.warn('MediaPipe Hands initialization failed:', e);
            showWarning('Hand detection not available. Other object detection will still work.');
            detectionToggles["Hand"] = false;
        }
        ...
        // Load COCO-SSD model for general object detection
        modelInfoElement.textContent = 'MODEL: LOADING COCO-SSD...';
        try {
            cocoSsdModel = await cocoSsd.load({
                base: performanceMode ? 'lite_mobilenet_v2' : 'mobilenet_v2'
            });
            ...
            console.error('COCO-SSD model loading failed:', e);
            showError('Object detection model failed to load. Please check your internet connection and reload the page.');
            throw e;
        }
        
    } catch (error) {
        console.error('Error loading models:', error);
        showError(`Failed to load detection models: ${error.message}`);
        throw error;
    }
}
